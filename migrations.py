...
choice_list = {'location': ['neyyar', 'leela', 'gayathri'],
               'processor': ['i3', 'i5', 'i7'],
               'ram': ['4', '8', '16'],
...
def random_alphanumeric(length=5):
    temp = ''.join(random.choice('0123456789ABCDEF') for i in range(length))
    return temp


def random_date(start_date='today', end_date='+1y'):
    # start_date = datetime.date(year=2015, month=1, day=1)
    temp = fake_obj2.date_between(start_date=start_date, end_date=end_date)
    return temp
